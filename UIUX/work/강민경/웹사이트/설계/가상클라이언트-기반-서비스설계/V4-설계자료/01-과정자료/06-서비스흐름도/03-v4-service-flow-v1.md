...
기존 3-B 청사진에서 발견된 DN-12/13 추적 항목을 복원하고, 강제 방문 및 자동 포커스 표현을 정제하여 사이트 수준 정보 탐색 동선으로 정리하였다.
...
| **PATH B** | 제품 사양 및 치수 탐색 | `PRODUCTS 1.1/1.2` | `V4-C01 (은재)`, `V4-C02 (지안)`, `V4-C04 (소담)`, `V4-C07 (다온)` | 제본/지질 사양, 실측 치수 대조, 자율 탐색 |
...
│  └─ PRODUCTS 1.1/1.2 ➔ 1.3 (스펙/치수 대조) ➔ (선택 1.4 탐색 기준 참고) ➔ Goal
...
| **B-2** | 스펙 & 치수 대조 | 소재, 평면 펼침, 필기 적합성, 실측 치수 정보 제공 | `PRODUCTS 1.3` | 정보 확인 | DECISION |
| **B-3** | 탐색 기준 안내 | 비교·탐색에 참고할 기준과 관련 제품 정보 범위 안내 | `PRODUCTS 1.4` | 탐색 기준 확인 | `PRODUCTS 1.3` 정보 비교 참고 |
...
4. **`V4-C04 (소담 - 글 보존)`**: `HOME 0.3` 진입 ➔ 글 원본 보존 경험 확인 ➔ **현재 사이트 범위의 상위 메시지 수용 (`INFORMATIONAL`)**. 독립 레이어 기능은 원본 서비스 요구로만 추적함.
5. **`V4-C05 (태오 - 오프라인 캐시)`**: 오프라인 캐시·로컬 소장은 원본 서비스 요구로 추적하되 현재 3 Page 사이트에서 기능이나 지원 정보로 제공하지 않음 (`OUT_OF_CURRENT_SCOPE_REFERENCE`).
6. **`V4-C06 (이준 - 기록 복귀)`**: `HOME 0.3` 진입 (Path A) ➔ 죄책감 없는 복귀 경험 파악 ➔ `BRAND 2.3` 기록 철학 읽기 ➔ **목표 달성 (`EXPERIENCE_PREVIEW`)**
7. **`V4-C07 (다온 - 자율 조건 탐색)`**: `PRODUCTS 1.4` 진입 ➔ 비실행형 탐색 기준과 관련 정보 범위 확인 ➔ `PRODUCTS 1.3` 소재·치수 대조 정보 참고 ➔ **목표 달성 (`DIRECT`)**
...
- **상황**: 사용자가 탐색 기준을 참고했지만 원하는 제품 정보를 찾기 어려운 경우.
- **대응**: 가입이나 진단을 강요하지 않으며, 전체 제품군 정보 또는 전체 스펙 대조 정보의 확인 경로를 안내함.
...
- **`HOME / MAIN`**: `0.1 First Impression`, `0.2 Brand Core Summary`, `0.3 Recording Experience`, `0.4 Product Groups Intro`, `0.5 Portfolio Showcase`, `0.6 Navigation & Footer` (6/6 섹션 수용)
- **`PRODUCTS / CATEGORY-SUB`**: `1.1 Visual Hero`, `1.2 Product Group Exploration`, `1.3 Product Feature & Specification`, `1.4 User-led Exploration` (4/4 섹션 수용)
- **`BRAND`**: `2.1 DAMORI Introduction`, `2.2 Brand Existence & Purpose`, `2.3 Recording Philosophy`, `2.4 Core Values & User Value`, `2.5 Analog & Digital Principle`, `2.6 Product Principle` (6/6 섹션 수용)
...
| **DN-05** | `RG-05` | `V4-C05` | 현재 경로 없음 | 현재 Page 직접 수용 없음 | 원본 서비스 요구 보존 · 현재 범위 밖 참고 |
| **DN-06** | `RG-06` | `V4-C06` | Path A, Path C | `HOME 0.3`, `BRAND 2.3` | 죄책감 없는 기록 복귀 철학 전달 |
| **DN-07** | `RG-07` | `V4-C07` | Path B | `PRODUCTS 1.4` | 비실행형 탐색 기준 및 관련 정보 범위 안내 |
...
| **DN-13** | `RG-04 관련` | `V4-C04` | Path C | `HOME 0.3` | 원본 서비스 요구로 추적하며 글 원본 보존 상위 메시지로 현재 범위 수용 |
...
2. `사용자 기준 탐색 정보 구조`: 가입·진단 없이 참고하는 비실행형 탐색 기준 안내
...
- **누락 검증**: 10개 Pure DN(`DN-01~08, 12, 13`)과 8개 RG(`RG-01~08`)의 현재 기준 추적 상태를 확인함.
- **새 Page 검사**: 공식 Page Anchor 3개(`HOME`, `PRODUCTS`, `BRAND`) 이외의 임의 신규 페이지 생성이 0건임.
- **GNB & Search**: SEARCH 노드는 현재 1차 GNB 및 Flow Node에서 0건임.
- **정합성**: 21개 필수 목차 및 7인 대표 클라이언트의 Site-level Journey 대응을 현재 기준에서 확인함.
