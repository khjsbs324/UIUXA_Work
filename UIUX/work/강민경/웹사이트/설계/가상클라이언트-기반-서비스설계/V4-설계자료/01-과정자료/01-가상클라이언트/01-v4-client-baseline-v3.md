...
- **Site-level Need (웹사이트 수용 요구)**: DAMORI가 제공하는 제품의 제본 특성(평면 펼침), 종이의 필기 적합성 정보, 브랜드의 제품 원칙을 탐색하고 실측 스펙을 대조함.
...
- **Product Requirements**: 180도 평면 펼침 제본 사양 검토, EF/F 필기 적합성 지질 검토, 두께 및 치수 실측 정보
- **Design Opportunities**: 제품 스펙 및 치수 1:1 대조 비교표 UI 후보
- **Global Rules**: ACCESSIBILITY_RULE, RESPONSIVE_RULE
- **Source Value Statuses**: [{"value": "180도", "context": "제본 펼침 각도", "status": "PRODUCT_REQUIREMENT_CANDIDATE", "note": "평면 펼침 사용자 요구 지원을 위한 제본 스펙 검토 조건"}, {"value": "120gsm", "context": "종이 평량 사양", "status": "PRODUCT_REQUIREMENT_CANDIDATE", "note": "필기 적합성을 위한 종이 사양 검토 조건"}, {"value": "0%", "context": "잉크 번짐률", "status": "USER_STATED_EXPECTATION", "note": "원본 사용자 기대 목표값이며 제품 보증값 아님"}]
...
- **Global Rules**: ACCESSIBILITY_RULE, RESPONSIVE_RULE
- **Source Value Statuses**: [{"value": "24시간", "context": "세로 타임라인 서식", "status": "PRODUCT_REQUIREMENT_CANDIDATE", "note": "시간축 플래너 레이아웃 구성 검토 조건"}, {"value": "1클릭", "context": "재설정 동작", "status": "OUT_OF_CURRENT_SCOPE_REFERENCE", "note": "원본 서비스 요구의 재설정 동작 수치이며 현재 웹사이트 기능 목표가 아님"}]
...
- **Site-level Need (웹사이트 수용 요구)**: 로컬 소장 및 오프라인 선호는 원본 서비스 요구로 보존하며, 현재 웹사이트에서는 실제 지원 기능으로 제공하지 않음.
- **Site Coverage Type**: `OUT_OF_CURRENT_SCOPE_REFERENCE`
- **Site Relevant Pages**: 현재 Page 직접 수용 없음
- **Scope Note**: Offline-First 작성, 로컬 저장 및 파일 추출은 현재 3 Page 포트폴리오 웹사이트 범위 밖이며 기능 지원을 약속하지 않음.
...
- **Design Opportunities**: 로컬 백업 안내 모듈은 현재 범위 밖 참고 요구
- **Global Rules**: ACCESSIBILITY_RULE, RESPONSIVE_RULE
...
- **Global Rules**: ACCESSIBILITY_RULE, RESPONSIVE_RULE
- **Source Value Statuses**: [{"value": "1-클릭", "context": "복귀 동선", "status": "OUT_OF_CURRENT_SCOPE_REFERENCE", "note": "원본 서비스 요구의 자동 복귀 수치이며 현재 웹사이트 기능 목표가 아님"}]
...
- **Site-level Need (웹사이트 수용 요구)**: DAMORI 제품군을 비교·탐색할 때 참고할 기준과 관련 정보 범위를 확인하고, 소재 및 실측 치수 대조 정보를 직접 살펴봄.
- **Site Coverage Type**: `DIRECT`
- **Site Relevant Pages**: PRODUCTS (1.3 스펙 대조 & 1.4 자율 탐색)
- **Scope Note**: PRODUCTS 1.4는 동적 필터가 아닌 비실행형 탐색 기준 안내이며, 실측 치수 대조 정보는 PRODUCTS 1.3에서 확인함.
...
- **순수 Product Pool 소스 (31명)**: `VC-10~13, 15, 19, 21~28, 31, 33~41, 43~49` (Primary Persona = NONE이며, 제품 인벤토리 대조 소스로 31/31 추적됨).
...
| **DN-05** | 오프라인 캐시 및 로컬 소장 | `V4-C05 (태오)` | 원본 서비스 요구 보존 · 현재 범위 밖 참고 |
| **DN-06** | 죄책감 없는 기록 복귀 | `V4-C06 (이준)` | 미안함 없는 기록 철학 및 복귀 경험 미리보기 |
| **DN-07** | 가입 없는 자율 탐색 | `V4-C07 (다온)` | 비실행형 탐색 기준 안내로 수용 |
...
| **RG-05** | 오프라인 데이터 소장 패턴 | `V4-C05` | 원본 서비스 요구 보존 · 현재 범위 밖 참고 |
| **RG-06** | 미안함 없는 복귀 지원 패턴 | `V4-C06` | HOME & BRAND 기록 철학 수용 |
| **RG-07** | 사용자 기준 자율 탐색 패턴 | `V4-C07` | PRODUCTS 비실행형 탐색 기준 안내로 수용 |
...
| **C05** | Offline-First, PDF/CSV 추출 | 로컬 소장 및 오프라인 안전성 | 원본 서비스 요구 보존 · 현재 기능 범위 밖 | `OUT_OF_CURRENT_SCOPE_REFERENCE` | `-` |
| **C06** | 1-클릭 작성 위치 포커스 복귀 | 미안함 없는 기록 재개 | 미안함 없는 복귀 철학 및 가이드 파악 | `EXPERIENCE_PREVIEW` | `HOME (0.3)` |
| **C07** | 4개 칩 자율 좁히기 및 1:1 대조 | 자율 탐색 및 스펙 비교 | 비실행형 탐색 기준 안내 & 치수 대조 정보 | `DIRECT` | `PRODUCTS (1.3/1.4)` |

## 12. 후속 설계 운영 규칙

1. **Traceability 준수**: 51개 Source VC는 V4 설계 과정 전반에서 51/51 추적 가능해야 함.
