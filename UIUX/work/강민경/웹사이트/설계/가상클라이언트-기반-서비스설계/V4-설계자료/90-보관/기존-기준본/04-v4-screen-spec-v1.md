...
- **버전**: `V1`
- **공식 브랜드명**: `DAMORI / 다모리`
- **상태**: `V4_SCREEN_SPEC_BASELINE_V1` (사용자 승인 공식 기준본)
...
4. **Grayscale 와이어프레임 대비성**: 색상에 의존하지 않고 정보의 크기, 여백, 대비, 그룹핑만으로 100% 정보 위계가 성립하도록 설계한다.
...
| **1.0** | `PAGE-01` | `PRODUCTS / CATEGORY-SUB` | 제품군 비주얼 탐색, 소재/제본/번짐/실측 치수 대조 및 자율 탐색 | `PATH B` (Product Task), `PATH C` (후속) |
...
2. **목적**: DAMORI 하이브리드 제품군을 포트폴리오 형태로 탐색하고, 소재/제본/번짐/실측 치수 대조 및 사용자 기준 자율 좁히기 정보를 제공함.
3. **주요 콘텐츠**:
   - `1.1 Visual Hero`: 카테고리/제품군 시각 아이덴티티 표현 영역.
   - `1.2 Product Group Exploration`: 약 200개 Raw Product Pool을 유연하게 수용하는 제품군 탐색 정보 영역.
   - `1.3 Product Feature & Specification`: 소재, 평면 펼침 제본, 잉크 번짐 저감, 실측 치수 1:1 대조 정보 및 사용 맥락(Usage Context) 안내.
   - `1.4 User-led Exploration`: 회원가입/진단 강요 없는 사용자 기준 자율 조건 좁히기 정보 영역.
4. **인터페이스**: GNB, 제품군 탐색 선택, 사양 대조 정보 확인 선택, 자율 조건 좁히기 정보 적용, `BRAND 2.6` 연결 링크.
5. **레이아웃**: 상단 비주얼 히로 ➔ 중단 제품군 탐색 갤러리 ➔ 하단 자율 조건 좁히기 정보 및 실측 치수 대조 정보 병렬 배치.
6. **작동**: 제품군 선택 시 관련 특징 정보 노출, 사용자 탐색 기준 적용 시 관련 제품 범위 확인 가능, 제품 원칙 클릭 시 `BRAND 2.6` 이동.
7. **연결**: `HOME (0.0)`, `BRAND (2.6)`
8. **사용자 경로**: `PATH B` (Product Task Flow: Step B-1 ~ B-5), `PATH C` (후속 단계)
9. **비고**: 결제, 장바구니, 가격, 수량 선택 등 쇼핑몰 UI 요소 100% 배제. 약 200개 Raw Product Pool을 수용하되 Category 수 및 이름은 미확정 유지함.

### 6.2 보조 QA 및 정합성 항목
- **관련 Client**: `V4-C01 (은재)`, `V4-C02 (지안)`, `V4-C04 (소담)`, `V4-C05 (태오)`, `V4-C07 (다온)`
- **관련 DN / RG**: `DN-01, 02, 04, 05, 07, 08, 13` / `RG-01, 02, 04, 05, 07, 08`
- **상태 및 예외**: 정상 스펙 대조 / 조건에 맞는 대상 부족 시 전체 제품군 안내 (`NO_RELEVANT_PRODUCT_INFORMATION`), 정보 누락 시 대체 정보 안내 (`PRODUCT_INFO_MISSING`).
...
- **Design Opportunity**: 소재 & 치수 1:1 대조 비교표 UI, 자율 탐색 매트릭스 UI.
- **확정 / 미확정 / 후속 결정**:
...
## 9. Client Coverage (7인 전원 100% 매핑)
...
| **V4-C05** | 태오 | 로컬 백업 지원 시스템 특성 안내 | `PRODUCTS 1.3` | - | **COVERED (`INFORMATIONAL`)** |
...
- **`PATH A (Main Narrative)`**: `HOME 0.1 ➔ 0.5` 브랜드 발견 및 서사 랜딩 화면 정보로 완전 대응됨.
- **`PATH B (Product Task)`**: `PRODUCTS 1.1 ➔ 1.3 (1.4)` 스펙 및 치수 대조 화면 정보로 완전 대응됨.
- **`PATH C (Need-based Discovery)`**: `HOME 0.3 ➔ PRODUCTS 1.2` 기록 경험 발견 화면 정보로 완전 대응됨.
...
| **1.0 PRODUCTS**| 1.1~1.4 섹션 역할, 스펙/치수 대조 및 자율 탐색 기능 목적 | Category 수/이름, 대표 15개 선정, Filter UI 형태 ➔ Wireframe 이관 |
...
- **파일 생성 검증**: `V4-설계자료/04-v4-screen-spec-v1.md` 1개 공식 마스터 파일 생성 완료.
- **선생님 9개 양식**: 3개 화면 모두 9/9 완벽 준수.
