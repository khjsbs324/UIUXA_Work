...
- **버전**: `V1`
- **공식 브랜드명**: `DAMORI / 다모리`
- **상태**: `V4_STORYBOARD_BASELINE_V1` (사용자 승인 공식 기준본)
...
4. **Grayscale 와이어프레임 전환성 원칙**: 브랜드 색상을 확정하지 않고 Dark Gray, Medium Gray, Light Gray, Black, White의 무채색 톤 대조만으로 100% 정보 위계가 구분되도록 설계한다.
...
| **1.0** | `PAGE-01` | `PRODUCTS / CATEGORY-SUB` | 하이브리드 제품군 비주얼 탐색 ➔ 소재/제본/번짐/치수 대조 ➔ 자율 좁히기 | `PATH B` (Product Task), `PATH C` (후속) |
...
- **Story Concept**: 포트폴리오 제품군 탐색 ➔ 소재/제본/번짐/실측 치수 대조 ➔ 사용자 기준 자율 좁히기
...
| **3** | `1.3` | Product Feature & Spec | 소재, 제본, 번짐, 실측 치수 대조 | 소재, 평면 펼침, 번짐 저감, 실측 치수 대조 정보 및 사용 맥락 | Spec & Material Detail Area | 스펙/치수 대조 | `1.4` / `BRAND 2.6` | PRIMARY | `OPEN-02` (대조 강조 방식) |
| **4** | `1.4` | User-led Exploration | 내 기준 자율 탐색 | 가입 없는 사용자 기준 자율 좁히기 정보 | User-led Exploration Area | 탐색 범위 좁히기 | `BRAND 2.6` | SUPPORT | 자율 탐색 UI 형태 (`DESIGN_CANDIDATE`) |
...
## 9. Client Coverage (7인 전원 100% 수용)
...
| **V4-C05** | 태오 | 로컬 백업 지원 시스템 특성 안내 | `PRODUCTS 1.3` | `INFORMATIONAL` | **COVERED** |
...
| **DN-05** | `RG-05` | `V4-C05` | `PRODUCTS 1.3` | 로컬 백업 지원 시스템 특성 안내 제공 | **TRACEABLE** |
| **DN-06** | `RG-06` | `V4-C06` | `HOME 0.3`, `BRAND 2.3` | 죄책감 없는 기록 복귀 철학 전달 | **TRACEABLE** |
| **DN-07** | `RG-07` | `V4-C07` | `PRODUCTS 1.4` | 사용자 기준 자율 조건 좁히기 정보 제공 | **TRACEABLE** |
...
| **DN-13** | `RG-04관련` | `V4-C04` | `PRODUCTS 1.3` | 스티커/프레임 독립 레이어 보존 안내 | **TRACEABLE** |

---

## 11. Service Flow 반영

- **`PATH A`**: `HOME 0.1 ➔ 0.5` 메인 서사 랜딩 콘텐츠 순서와 완전 대응됨.
- **`PATH B`**: `PRODUCTS 1.1 ➔ 1.3 (1.4)` 태스크/스펙 대조 콘텐츠 순서와 완전 대응됨.
- **`PATH C`**: `HOME 0.3 ➔ 0.4 ➔ PRODUCTS 1.2 ➔ 1.3` 경험 발견 콘텐츠 순서와 완전 대응됨.

---

## 12. Screen Specification 정합성

- 공식 Screen Spec V1의 3대 화면 9개 필수 양식과 100% 일치하며, 정보 역할(WHAT)을 시각적 콘텐츠 순서(HOW)로 충돌 없이 확장함.
...
|---|---|---|:---:|---|
| **`OPEN-01`** | `HOME 0.1` | Editorial Brand Narrative Visual vs Product Object Visual 우선순위 | `OPEN_VISUAL_DECISION` | Wireframe / Visual 단계 |
| **`OPEN-02`** | `PRODUCTS 1.3` | Material Detail 강조 vs Specification Data 강조 vs Balanced 균형 | `OPEN_VISUAL_DECISION` | Wireframe / Visual 단계 |
| **`CANDIDATE-01`** | `PRODUCTS 1.3` | 소재/제본/치수 1:1 대조 비교표 시각 구획 방식 | `DESIGN_CANDIDATE` | Wireframe / Visual 단계 |
| **`CANDIDATE-02`** | `PRODUCTS 1.4` | 사용자 기준 자율 좁히기 정보 매트릭스 UI | `DESIGN_CANDIDATE` | Wireframe / Visual 단계 |
...
- **와이어프레임 무채색 제약**: 후속 와이어프레임은 **WHITE, GRAY, BLACK Grayscale만 사용**하며, 브랜드 색상에 의존하지 않고 정보의 크기, 위치, 여백, 대비, 그룹핑만으로 100% 정보 위계가 표현되도록 설계함.
...
|---|---|---|
| **0.0 HOME** | 0.1~0.6 섹션 역할, 포트폴리오 랜딩 성격, 대표 제품 최소 15개 수용 구조 | exact px, Column 수, 애니메이션 시간, `OPEN-01` 시각 톤 ➔ Wireframe 이관 |
| **1.0 PRODUCTS**| 1.1~1.4 섹션 역할, 스펙/치수 대조 및 자율 탐색 기능 목적 | Category 수/이름, 대표 15개 선정, `OPEN-02` 대조 방식, Filter UI ➔ Wireframe 이관 |
...
- **파일 생성 검증**: `V4-설계자료/05-v4-storyboard-v1.md` 1개 공식 마스터 파일 생성 완료.
- **Traceability 검증**: 7인 Client, 10개 Pure DN, 8개 RG 100% 매핑 완수.
- **과설계 배제 검증**: exact Gray HEX 0건, exact Column 수 0건, 1:1 표/컴포넌트 강제 0건 준수.
- **현재 문서 상태**: **`V4_STORYBOARD_BASELINE_V1`**
