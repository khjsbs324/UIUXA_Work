...
- **버전**: `V3`
- **공식 브랜드명**: `DAMORI / 다모리`
- **상태**: `V4_SITE_MAP_BASELINE_V3` (사용자 승인 공식 기준본)
...
### 1.2 V2 ➔ V3 주요 보완사항
1. **필수 16개 목차 완벽 복원**: 주요 사용자 경로(User Journey), 설계 근거, 핵심 사용자 목표, 가정 및 확인 필요사항 등 16개 필수 목차를 전면 복원함.
2. **C05 Site Scope 재정정**: C05의 주 수용 위치를 `PRODUCTS` (1.3 시스템 지원 안내)로 정제하고, `HOME Footer` 고정 배치를 배제함.
3. **Site-level User Journey 복원**: 7인 대표 클라이언트의 여행 경로를 포트폴리오 웹사이트 수준의 목표 달성 기준으로 완전 작성함.
4. **Canonical Label 통일**: MMD와 SVG 다이어그램의 라벨을 마크다운 명세의 한국어 명칭과 1:1 완전 일치시킴.
...
- `V4-C05 (태오)`: `PRODUCTS` (로컬 백업 지원 시스템 안내)
...
- 제품 소재, 제본, 잉크 번짐 저감 및 실측 치수 대조 정보 확인.
...
1. **`V4-C01 (은재 - 장문 사색)`**: `HOME` ➔ `PRODUCTS (1.3 제본 및 번짐 저감 스펙 대조)` ➔ `BRAND (2.6 제품 원칙 확인)` ➔ **사이트 수준 정보 탐색 목표 달성**
...
5. **`V4-C05 (태오 - 오프라인)`**: `HOME` ➔ `PRODUCTS (1.3 오프라인 및 로컬 백업 지원 특성 안내 확인)` ➔ **사이트 수준 시스템 지원 정보 파악 목표 달성**
...
- **누락 검증**: 필수 16개 목차가 완벽히 수용되었으며 V4 7인 대표 클라이언트의 Site-level Journey 반영 누락 0건임.
